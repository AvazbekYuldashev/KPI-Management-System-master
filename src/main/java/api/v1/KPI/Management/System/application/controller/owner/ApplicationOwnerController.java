package api.v1.KPI.Management.System.application.controller.owner;
...
import api.v1.KPI.Management.System.application.service.owner.ApplicationOwnerService;
...
@RestController
@RequestMapping("/api/v1/application-owner")
public class ApplicationOwnerController {
    ...
    public ResponseEntity<Page<ApplicationResponseDTO>> filter(@RequestBody ApplicationFilterDTO dto,
        ...
        return ResponseEntity.ok().body(applicationOwnerService.searchApplications(dto, getCurrentPage(page), size, lang));
    }

    @PatchMapping("/status")
    public ResponseEntity<AppResponse<String>> updateStatus(@RequestBody ApplicationStatusDTO dto,
                                                            @RequestHeader(value = "Accept-Language", defaultValue = "UZ") AppLanguage lang) {
        return ResponseEntity.ok().body(applicationOwnerService.updateStatus(dto, lang));
    }
